...
Obwohl diese Darstellung mittlerweile mehr als zwanzig Jahre alt ist, ist der generelle Ablauf immer noch gültig und wird bei der Planung eines Data Warehouse beachtet. Die ersten drei Kernkompetenzen werden bereits bei der Architektur des Data Warehouse berücksichtigt, die Transformation der Daten erfolgt beim Durchlaufen der Schichten eines Data Warehouse (z.~B. _Staging_ #sym.arrow _Core Warehouse Layer_ #sym.arrow _Data Mart_).
...
/ RAG für Textgenerierung: Der Inhalt der PDFs wird abschnittsweise in Vektoren eingebettet. So können Daten gezielt abgerufen werden, um Fragen präzise zu beantworten.
...
Ein perfekter Chatbot mit optimaler RAG für die Textgenerierung würde vermutlich viele fachliche Argumente gegen die Verwendung von generativer KI in der Wissenschaft außer Kraft setzen. Trotzdem sprechen moralische Gründe dafür, Ideen selbst zu entwickeln und Publikationen selbst zu schreiben @aylsworth_should_2024. Aus diesem Grund wird für diesen Programmentwurf RAG für Dokumentensuche, also die Unterstützung bei der Recherche für diese Publikationen, und SPECTER als Embeddingmodell für die RAG-Pipeline ausgewählt.
...
Alternativ zur Verwendung von Python auf dem Entwicklungsrechner kann auch ein Development Container mit Docker aufgesetzt werden. Die Verwendung solcher Container stellt sicher, dass Projekte voneinander isoliert ausgeführt werden. Wenn auch PostgreSQL als Container ausgeführt wird, kann die Container-Engine das Routing zwischen den Containern übernehmen. Aus diesem Grund wird für diesen Programmentwurf ein vorgefertigtes Dev-Environment aus Docker-Containern mit Python und PostgreSQL verwendet, das sich in der IDE PyCharm einfach erstellen lässt -- vorausgesetzt, Docker (oder ein Drop-In-Replacement) ist installiert. Der Ablauf ist wie folgt:

1. Nach dem Öffnen von PyCharm wird auf dem Welcome-Bildschirm links der Menüpunkt _Remote Development_ ausgewählt.
2. Ein Klick auf _Create Dev Container_ öffnet das Setup-Menü. Dort steht zur Auswahl, welche Container-Engine verwendet werden soll, mit welcher JetBrains-IDE gearbeitet wird und welches entfernte Git-Repository auf welchem Branch als Grundlage für den Code verwendet werden soll. Alternativ kann auch ein lokales Projekt angegeben werden.
...
Die Nutzung eines solchen Containers ist nicht zwingend notwendig, alternativ kann auch z.~B. in einem Virtual Environment (`venv`) entwickelt werden. Im Quellcode muss die Datenbankverbindung dann unbedingt angepasst werden, um statt auf den `db`-Container auf die stattdessen verwendete PostgreSQL-Instanz zu verweisen.
...
Nach Abschluss der Installation kann mit der Arbeit am eigentlichen Programmentwurf begonnen werden.
Für die erfolgreiche Umsetzung sind vier Schritte zu erledigen: Zunächst muss ein Datenset mit den zu speichernden PDF-Dateien erstellt werden,
danach muss aus jedem dieser Dokumente der Titel und das Abstract extrahiert und gespeichert werden.
Im dritten Schritt durchlaufen die so vorbereiteten Daten den RAG-Import-Flow aus @grundlagen (@rag-flow_import) und werden in der Datenbank gespeichert.
Im letzten Abschnitt wird die Suche nach Dokumenten realisiert.

== Erstellung des Datensets

Um die Datengrundlage für das Einlesen der Dokumente in die Datenbank zu bewerkstelligen, wurden 51 Preprints im PDF-Format von arXiv heruntergeladen. Da diese im Git-Repository hinterlegt werden, wurde darauf geachtet, dass die Größe eines Artikels 3.5 MB nicht überschreitet. In einem produktiven Einsatz der Anwendung sollte es problemlos möglich sein, auch größere PDFs zu verarbeiten. Darüber hinaus wurden Preprints aus verschiedenen Kategorien (hauptsächlich Informatik, Astrophysik, Elektrotechnik und Wirtschaft) heruntergeladen, die sich thematisch teilweise drastisch unterscheiden, teilweise aber auch überschneiden. Die Preprints wurden unkatalogisiert und unter ihrem Download-Dateinamen (z. B. `2405.00695v1.pdf`) im Verzeichnis `arxiv-pdfs` abgelegt.
...
Das Sichern der auf diese Weise erzeugten Embedding-Vektoren kann unter Einsatz der Python-Bibliothek `psycopg2` durch einfache SQL-Statements vorgenommen werden. Für den Produktiveinsatz ist die implementierte Vorgehensweise nicht geeignet, da sie für SQL-Injection-Angriffe anfällig ist, allerdings bestehen für dieses Proof-of-Concept keine hohen Sicherheitsanforderungen. Als weitere Vereinfachung wird die Standard-Datenbank `postgres` verwendet, das kann bei Bedarf in der Verbindungskonfiguration angepasst werden (s. @psycop_config). Die restlichen Konfigurationsdaten sind ebenfalls auf den in @dev-container-setup eingerichteten Dev-Container abgestimmt.
...
Die in @prepare-for-import beschriebenen Probleme im Umgang mit PDFs sind beim Abrufen nicht relevant, daher kann der ursprünglich geplante Ablauf aus @rag-flow_retrieval problemlos umgesetzt werden. Zur Vereinfachung der Nutzung wird die RAG-Anfrage (Prompt/Query), direkt als Kommandozeilenparameter an das Python-Skript `rag-retrieval.py` übergeben. Dieser Prompt wird dann mit SPECTER in einen Vektor kodiert, siehe @query-encoding. Danach werden über die Ähnlichkeitssuche von pgVector die fünf Dokumente ermittelt, die am ehesten zur Nutzereingabe passen. Dafür wird das SQL-Skript in @sql-similarity-search durch `psycopg2` ausgeführt. Die Ähnlichkeitssuche wird durch den pgVector-Operator `<->` angewendet.
...
Trotz des recht einfach gehaltenen Skripts sind die Ergebnisse sehr gut: @ergebnis_rag_retrieval zeigt einen beispielhaften Aufruf und die vom Skript gefundenen Dokumente für den Suchbegriff "Retrieval Augmented Generation in Data Warehouses". Die ersten vier Ergebnisse waren erwartbar, da sie alle RAG als Thema haben und dies auf der ersten Seite mehrfach nennen, aber der fünfte Preprint enthält weder das Schlagwort #text(lang: "en")[_Retrieval Augmented Generation_] noch _Data Warehouse_, ist aber -- wie das Skript bzw. das Embedding-Modell korrekt erkannt hat -- thematisch sehr ähnlich. Damit ist die Einsetzbarkeit des RAG-Workflows bewiesen, da von den Skripten aus der Preprint-Datenbank thematisch passende Artikel empfohlen werden, die teilweise über eine reine Wortsuche nicht auffindbar gewesen wären.

= Hinweis zur Verwendung von generativer KI

Aufgrund der geringen Erfahrung des Autors wurde generative KI eingesetzt, um bestimmte Sprachkonstrukte zu recherchieren. Außerdem wurde das Skript zum Testen der beiden PDF-Bibliotheken von ChatGPT erstellt, da es keinen direkten Bezug zum Thema der Arbeit hat und die Python-Kenntnisse des Autors nicht stark genug ausgeprägt sind, um ein solches Skript selbst zu erstellen. Für das Schreiben dieser Dokumentation wurde keine generative KI eingesetzt. Bei der Erstellung der Skripte `rag-import.py` und `rag-retrieval.py` wurde generative KI nur beratend bei Python-Fragen eingesetzt.